...
class ApiClient {
  constructor() {
    ...
    this.activeRequests = 0; // Contador de peticiones activas
  }
  ...
  async request(endpoint, options = {}) {
    ...
    // Iniciar loader si no se especifica skipLoader
    this.startRequest(skipLoader);
          ...
          try {
            ...
            if (!retryResponse.ok) {
              const retryErrorData = await retryResponse
                ...
                .catch(() => ({}));
              throw new Error(
                retryErrorData.message || `Error HTTP ${retryResponse.status}`
              );
            }
