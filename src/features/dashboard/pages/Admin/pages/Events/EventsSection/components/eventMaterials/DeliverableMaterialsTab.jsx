...
const DeliverableMaterialsTab = ({
  ...
  const quantityValidation = useMemo(() => {
    ...
    if (qty > available) return { valid: false, message: `Stock insuficiente. Disponible: ${available}` };
  ...
  const handleAddMaterial = () => {
    ...
    if (isAlreadyAssigned) {
      showWarningAlert(
        ...
        `${selectedMaterialData?.nombre || "este material"} ya está en la lista. Si deseas cambiar la cantidad, usa el botón de editar.`,
      );
      return;
    }
    ...
    if (isInPending) {
      showWarningAlert(
        ...
        `${selectedMaterialData?.nombre || "este material"} ya está en la lista de pendientes. Si deseas cambiar la cantidad, elimínalo y agrégalo nuevamente.`,
      );
      return;
    }

    onAddMaterial({ materialId: materialId, cantidad: parseInt(quantity), observaciones: note });
    setSelectedMaterial("");
    setQuantity("");
    setNote("");
  };
  ...
  const editQuantityValidation = useMemo(() => {
    if (!editQuantity || !editingItem) return { valid: false, message: "" };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await EventMaterialsService.removeConsumable(editingItem.assignments[0].id);
  ...
  const getTotals = () => {
    ...
    const totalUnits = deliverables.reduce((sum, item) => sum + item.qty_total, 0);
    const totalDonated = deliverables.reduce((sum, item) => sum + item.qty_donated, 0);
    const totalManual = deliverables.reduce((sum, item) => sum + item.qty_manual, 0);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="text-2xl font-semibold text-gray-900">{totals.totalDonated}</div>
        </div>
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="text-sm text-gray-600">Stock Evento</div>
          <div className="text-2xl font-semibold text-gray-900">{totals.totalManual}</div>
        </div>
      </div>
      ...
      {!isReadOnly && (
        <div className="bg-gray-50 rounded-lg p-4 space-y-3 border-2 border-primary-purple/20">
          <h3 className="font-medium text-gray-900">Agregar Material desde Stock Evento</h3>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <span className="text-xs text-gray-500 ml-2">
                  ...
                  {editingItem.stock_available} disponibles + {editingItem.qty_manual} actuales)
                </span>
              </label>
              <input
                type="number"
      ...
      {deliverables.length === 0 && pendingMaterials.length === 0 ? (
        <div className="text-center py-12 bg-slate-50 rounded-lg">
          ...
          <p className="text-sm text-slate-500 mt-1">
            Agrega materiales desde stock evento o se cargarán automáticamente desde donaciones
          </p>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-4 py-3 text-center text-sm font-medium text-slate-700">Donado</th>
                <th className="px-4 py-3 text-center text-sm font-medium text-slate-700">Stock Evento</th>
                <th className="px-4 py-3 text-center text-sm font-medium text-slate-700">Total</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {deliverables.map((item) => (
                <tr
                  ...
                  className={item.is_locked ? "bg-slate-50" : "bg-white hover:bg-slate-50"}
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {item.is_locked && <Lock className="w-4 h-4 text-slate-400" />}
                      <div>
                  ...
                  <td className="px-4 py-3 text-center">
                    {item.qty_manual > 0 ? (
                      <span className="text-slate-900 font-medium">{item.qty_manual}</span>
                    ) : (
                      <span className="text-slate-400">-</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className="font-semibold text-slate-900">{item.qty_total}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-sm text-slate-600">{item.assignments?.[0]?.note || "-"}</span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    {item.qty_manual > 0 && item.assignments?.[0]?.id && !isReadOnly ? (
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          onClick={() => handleRemoveManual(item.assignments[0].id, item.material_name, item.qty_manual)}
