...
import { UserCheck, Users, X as XIcon, Trash2, ChevronDown, Plus } from "lucide-react";
  ...
  const validateTeamConsistency = () => {
    if (selectedTrainer && selectedPlayers.length > 0) {
      ...
      if (trainerType !== playersType) {
        const trainerTypeText = trainerType === "fundacion" ? "Fundación" : "Temporal";
        const playersTypeText = playersType === "fundacion" ? "Fundación" : "Temporal";
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      {selectedTrainer ? (
                        ...
                        <p className="text-gray-500 group-hover:text-gray-700">Seleccionar entrenador</p>
                      )}
                    </div>
                  </div>
                  <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform ${
                    selectedTrainer ? "text-primary-purple" : "group-hover:text-primary-purple"
                  }`} />
                </button>
                ...
                <div className="flex items-center gap-2">
                  {selectedPlayers.length > 0 && (
                    <span className="text-sm text-primary-purple font-semibold bg-primary-purple/10 px-3 py-1 rounded-full">
                      {selectedPlayers.length} seleccionada{selectedPlayers.length !== 1 ? 's' : ''}
                    </span>
                  )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <p className={`font-medium ${
                        selectedPlayers.length > 0 ? "text-gray-900" : "text-gray-500 group-hover:text-gray-700"
                      }`}>
                        {selectedPlayers.length > 0 ? "Modificar selección de jugadoras" : "Seleccionar jugadoras"}
                      </p>
                      {selectedPlayers.length === 0 && (
                        <p className="text-sm text-gray-400 mt-1">Haz clic para agregar jugadoras al equipo</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <Plus className={`w-5 h-5 text-gray-400 transition-transform ${
                      selectedPlayers.length > 0 ? "text-primary-purple" : "group-hover:text-primary-purple"
                    }`} />
                  </div>
                </button>
                ...
                {selectedPlayers.length > 0 && (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-gray-700">Jugadoras seleccionadas</span>
