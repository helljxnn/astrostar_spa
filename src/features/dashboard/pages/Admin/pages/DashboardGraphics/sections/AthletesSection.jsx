...
const AthletesSection = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      let paymentsResponse = {
        success: false,
        data: { payments: [], stats: { total: 0, thisMonth: 0 } },
      };
      ...
      let attendanceResponse = {
        success: false,
        data: {
          stats: { averageAttendance: 0, attendanceRate: 0, totalRecords: 0 },
        },
      };
      ...
      if (enrollmentsResponse.success && enrollmentsResponse.data) {
        ...
        enrollments.forEach((enrollment, index) => {
          const estado =
            enrollment.estadoMatricula ||
            enrollment.estado ||
            enrollment.latestEnrollment?.estado;
      ...
      if (athletesResponse.success && athletesResponse.data) {
        ...
        athletes.forEach((athlete, index) => {
          ...
          if (categoria.includes("infantil")) {
            ...
            categoria.includes("pre") &&
            categoria.includes("juvenil")
          ) {
            preJuvenil++;
  ...
  const totalByCategory =
    categoryStats.infantil + categoryStats.preJuvenil + categoryStats.juvenil;
  const infantilPercentage =
    totalByCategory > 0 ? (categoryStats.infantil / totalByCategory) * 100 : 0;
  const preJuvenilPercentage =
    totalByCategory > 0
      ? (categoryStats.preJuvenil / totalByCategory) * 100
      : 0;
  const juvenilPercentage =
    totalByCategory > 0 ? (categoryStats.juvenil / totalByCategory) * 100 : 0;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-br from-primary-purple/10 to-primary-purple/5 rounded-2xl shadow-lg p-6 border-2 border-primary-purple/20 hover:border-primary-purple/40 transition-all">
          ...
          <p className="text-4xl font-bold bg-gradient-to-r from-primary-purple to-purple-600 bg-clip-text text-transparent">
            {loading
              ? "..."
              : kpis.activeEnrollments > 0
                ? `${Math.round((kpis.activeEnrollments / kpis.totalAthletes) * 100)}%`
                : "0%"}
          </p>
