...
const generatePaymentsData = (paymentsData) => {
  ...
  if (paymentsData && Array.isArray(paymentsData)) {
    // console.log('🔍 Procesando', paymentsData.length, 'pagos...');
    ...
    paymentsData.forEach((payment, index) => {
      ...
      if (year === currentYear && paymentsByMonth[monthIndex]) {
        ...
        // console.log(`📊 Pago ${index + 1}: fecha=${paymentDate.toLocaleDateString()}, estado=${status}`);
        
        paymentsByMonth[monthIndex].total++;
        
        if (status.includes('aprobado') || status.includes('approved') || status.includes('completado') || status.includes('completed')) {
          paymentsByMonth[monthIndex].aprobados++;
        } else if (status.includes('pendiente') || status.includes('pending') || status.includes('en_proceso')) {
          paymentsByMonth[monthIndex].pendientes++;
        } else if (status.includes('rechazado') || status.includes('rejected') || status.includes('cancelado') || status.includes('cancelled')) {
          paymentsByMonth[monthIndex].rechazados++;
        } else {
          // Si no reconocemos el estado, contar como pendiente
          console.warn('⚠️ Estado no reconocido:', status, 'contando como pendiente');
          paymentsByMonth[monthIndex].pendientes++;
        }
      }
    });
  ...
  console.log('📈 Datos finales por mes:', data);
  return data;
};
...
const PaymentsGraphic = () => {
  ...
  const fetchPaymentsData = async () => {
    try {
      ...
      // console.log('📊 Respuesta completa de pagos:', response);

      if (response.success && response.data && response.data.payments) {
        // console.log('📊 Procesando', response.data.payments.length, 'pagos...');
        const generatedData = generatePaymentsData(response.data.payments);
        setData(generatedData);
        console.log('📈 Datos procesados para gráfica:', generatedData);
      } else {
  ...
  const maxValue = Math.max(...data.map(item => Math.max(item.aprobados, item.pendientes, item.rechazados, item.total)));
      ...
      <div className="w-full overflow-x-auto">
        {loading ? (
          ...
          <div className="relative" style={{ minWidth: `${chartWidth}px`, height: '350px' }}>
            ...
            {/* Grid lines */}
            <defs>
              <pattern id="grid-payments" width={spacing} height="50" patternUnits="userSpaceOnUse">
                <path d={`M ${spacing} 0 L 0 0 0 50`} fill="none" stroke="#e5e7eb" strokeWidth="1" strokeDasharray="3,3"/>
              </pattern>
            </defs>
            <rect width={chartWidth} height="270" fill="url(#grid-payments)" y="20"/>

            {/* Y axis labels */}
            <text x="15" y="30" fontSize="11" fill="#6b7280">{maxValue}</text>
            <text x="15" y="80" fontSize="11" fill="#6b7280">{Math.round(maxValue * 0.8)}</text>
            <text x="15" y="130" fontSize="11" fill="#6b7280">{Math.round(maxValue * 0.6)}</text>
            <text x="15" y="180" fontSize="11" fill="#6b7280">{Math.round(maxValue * 0.4)}</text>
            <text x="15" y="230" fontSize="11" fill="#6b7280">{Math.round(maxValue * 0.2)}</text>
            ...
            <polyline
              points={data.map((item, index) => {
                const x = 60 + (index * spacing);
                const y = (getYPosition(item.pendientes, maxValue) * 2.5) + 20;
                return `${x},${y}`;
              }).join(' ')}
              fill="none"
              stroke="#f59e0b"
            ...
            <polyline
              points={data.map((item, index) => {
                const x = 60 + (index * spacing);
                const y = (getYPosition(item.rechazados, maxValue) * 2.5) + 20;
                return `${x},${y}`;
              }).join(' ')}
              fill="none"
              stroke="#ef4444"
              ...
              return (
                <g key={index}>
                  <circle
                    ...
                    style={{ 
                      cursor: 'pointer',
                      filter: hoveredPoint?.index === index && hoveredPoint?.type === 'aprobados' ? 'drop-shadow(0 0 4px rgba(16, 185, 129, 0.8))' : 'none' 
                    }}
                    onMouseEnter={() => setHoveredPoint({ index, type: 'aprobados', value: item.aprobados, mes: item.mes, x })}
                  ...
                  <circle
                    ...
                    style={{ 
                      cursor: 'pointer',
                      filter: hoveredPoint?.index === index && hoveredPoint?.type === 'pendientes' ? 'drop-shadow(0 0 4px rgba(245, 158, 11, 0.8))' : 'none' 
                    }}
                    onMouseEnter={() => setHoveredPoint({ index, type: 'pendientes', value: item.pendientes, mes: item.mes, x })}
                    onMouseLeave={() => setHoveredPoint(null)}
                  />
                  <circle
                    cx={x}
                    ...
                    style={{ 
                      cursor: 'pointer',
                      filter: hoveredPoint?.index === index && hoveredPoint?.type === 'rechazados' ? 'drop-shadow(0 0 4px rgba(239, 68, 68, 0.8))' : 'none' 
                    }}
                    onMouseEnter={() => setHoveredPoint({ index, type: 'rechazados', value: item.rechazados, mes: item.mes, x })}
                    onMouseLeave={() => setHoveredPoint(null)}
                  />
                </g>
              );
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <div 
                    ...
                    style={{
                      backgroundColor: hoveredPoint.type === 'aprobados' ? '#10b981' : 
                                     hoveredPoint.type === 'pendientes' ? '#f59e0b' : '#ef4444'
                    }}
                  ></div>
